...
//SAVE AND LOAD
	ofstream ofichier ("test-loop.txt", ios::out | ios::app);
  ...
  loopCharRange->save(ofichier, savePosition);
	...
	ifstream ifichier ("test-loop.txt", ios::in);
	const shared_ptr<Loop> loopCarLoaded = Loop::load(ifichier, loadPosition);
	const shared_ptr<Loop> loopSelLoaded = Loop::load(ifichier, loadPosition);
	const shared_ptr<Loop> loopExSelLoaded = Loop::load(ifichier, loadPosition);
	const shared_ptr<Loop> loopSequenceLoaded = Loop::load(ifichier, loadPosition);
	const shared_ptr<Loop> loopLiteralLoaded = Loop::load(ifichier, loadPosition);
  const shared_ptr<Loop> loopCharRangeLoaded = Loop::load(ifichier, loadPosition);
  const shared_ptr<Loop> loopRecPointerLoaded = Loop::load(ifichier, loadPosition);
