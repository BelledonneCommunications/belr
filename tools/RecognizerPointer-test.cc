...
int main(int argc, char *argv[]){
	...
	//verification avec feed
		...
		if ((recognizptr5->feed(NULL, "a", 0) == 1) && (recognizptr5->feed(NULL, "b", 0) == 1) && (recognizptr5->feed(NULL, "abcd", 0) == 1) && (recognizptr5->feed(NULL, "abcd", 0) == 1))cout << "SUCESS : Literal RecognizerPointer feed successfull" << endl;
	  ...
	  //SAVE AND LOAD
		ofstream ofichier ("test-recon.txt", ios::out | ios::app);
	  recognizptr1->save(ofichier, savePosition);
		...
		ifstream ifichier ("test-recon.txt", ios::in);
		const shared_ptr<RecognizerPointer> recognizptrLoaded1 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded2 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded3 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded4 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded5 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded6 = RecognizerPointer::load(ifichier , loadPosition);
		const shared_ptr<RecognizerPointer> recognizptrLoaded7 = RecognizerPointer::load(ifichier , loadPosition);
		...
		if ((recognizptrLoaded5->feed(NULL, "a", 0) == 1) && (recognizptrLoaded5->feed(NULL, "b", 0) == 1) && (recognizptrLoaded5->feed(NULL, "abcd", 0) == 1) && (recognizptrLoaded5->feed(NULL, "abcd", 0) == 1))cout << "SUCESS : Literal RecognizerPointer feed successfull" << endl;
		else cout << "FAILED : CharRange RecognizerPointer feed gone wrong" << endl;
