...
int main(int argc, char *argv[]){
	...
	//verification avec feed
	  ...
	  //SAVE AND LOAD
		ofstream ofichier ("test-exsel.txt", ios::out | ios::app);
	  selector1->save(ofichier, savePosition);
		...
		ifstream ifichier ("test-exsel.txt", ios::in);
		const shared_ptr<Selector> selectorLoaded1 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded2 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded3 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded4 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded5 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded6 = ExclusiveSelector::load(ifichier , loadPosition);
		const shared_ptr<Selector> selectorLoaded7 = ExclusiveSelector::load(ifichier , loadPosition);
